...
void TrackView::resized()
{
    ...
    if (! isDrummer)
    {
        ...
        importButton.setBounds (area.removeFromLeft (56));
    }
...
void TrackView::openImportDialog()
{
    ...
    fileChooser->launchAsync (flags, [audioTrack] (const juce::FileChooser& fc)
    {
        const auto results = fc.getResults();
        if (results.isEmpty()) return;

        if (! audioTrack->loadFromFile (results[0]))
